...
public class AsyncIntegrationTest extends AbstractIntegrationTest {
  ...
  public void testGet() throws IOException {
    ...
    URI target = getUri().resolve("/async");

    assertThat(target.getPath(), containsString("/async"));

    HttpURLConnection http = HttpUrlUtil.openTo(target);
    assertThat(http.getResponseCode(), is(200));
    String responseBody = HttpUrlUtil.getResponseBody(http);
    assertThat(responseBody, containsString("Async is started."));
  }
